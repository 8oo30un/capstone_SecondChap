# Page snapshot

```yaml
- generic [ref=e1] [cursor=pointer]:
  - generic [ref=e2] [cursor=pointer]:
    - generic [ref=e3] [cursor=pointer]:
      - progressbar [ref=e5] [cursor=pointer]
      - main [ref=e13] [cursor=pointer]:
        - generic [ref=e14] [cursor=pointer]:
          - generic [ref=e17] [cursor=pointer]:
            - img [ref=e19] [cursor=pointer]
            - generic [ref=e24] [cursor=pointer]: Sign in with Google
          - generic [ref=e25] [cursor=pointer]:
            - heading "Sign in" [level=1] [ref=e26] [cursor=pointer]:
              - generic [ref=e27] [cursor=pointer]: Sign in
            - generic [ref=e29] [cursor=pointer]:
              - text: to continue to
              - button "secondchap" [ref=e30] [cursor=pointer]
        - generic [ref=e39] [cursor=pointer]:
          - generic [ref=e44] [cursor=pointer]:
            - textbox "Email or phone" [active] [ref=e45]
            - generic: Email or phone
          - button "Forgot email?" [ref=e49] [cursor=pointer]
        - generic [ref=e51] [cursor=pointer]:
          - button "Next" [ref=e55]:
            - generic [ref=e58]: Next
          - button "Create account" [ref=e63]:
            - generic [ref=e66]: Create account
    - contentinfo [ref=e69] [cursor=pointer]:
      - combobox "Change language English (United States)" [ref=e73] [cursor=pointer]:
        - generic:
          - generic: English (United States)
        - generic:
          - img
      - list [ref=e75] [cursor=pointer]:
        - listitem [ref=e76] [cursor=pointer]:
          - link "Help" [ref=e77]:
            - /url: https://support.google.com/accounts?hl=en-US&p=account_iph
        - listitem [ref=e78] [cursor=pointer]:
          - link "Privacy" [ref=e79]:
            - /url: https://accounts.google.com/TOS?loc=KR&hl=en-US&privacy=true
        - listitem [ref=e80] [cursor=pointer]:
          - link "Terms" [ref=e81]:
            - /url: https://accounts.google.com/TOS?loc=KR&hl=en-US
  - iframe [ref=e82] [cursor=pointer]:
    
```